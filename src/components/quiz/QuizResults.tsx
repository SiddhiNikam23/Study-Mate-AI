...
export default function QuizResults({
  ...
  return (
    <div>
      {/* Score card */}
      <div className="bg-white border border-blue-100 rounded-2xl p-6 shadow-sm text-center mb-6">
        ...
        <div className="text-slate-600 text-sm">
          {correct} / {attempts.length} correct on <span className="text-slate-900">{topic}</span>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 flex gap-3">
        ...
        <div>
          <p className="text-sm text-blue-700 font-medium">Memory updated</p>
      ...
      {wrong.length > 0 && (
        <div className="bg-white border border-blue-100 rounded-2xl p-5 shadow-sm mb-6">
          <div className="flex items-center justify-between gap-3 mb-3">
            <h2 className="font-semibold text-slate-900">Adaptive Recovery Plan</h2>
          ...
          {buildingRemediation ? (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
              <p className="text-sm text-blue-700 font-medium">Building your personalised practice...</p>
            ...
            <>
              {remediation?.weakTopics?.length ? (
                <div className="mb-4">
                  <p className="text-xs text-slate-600 mb-2">Weak topics detected</p>
                  <div className="flex flex-wrap gap-2">
                    {remediation.weakTopics.map((weakTopic) => (
                      <span
                        key={weakTopic}
                        className="text-xs bg-red-50 text-red-700 border border-red-200 px-2 py-1 rounded-lg"
                      >
                        {weakTopic}
                      </span>
                    ))}
              ...
              {remediation?.studyGuide && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-4">
                  <p className="text-sm font-medium text-blue-700 mb-2">Study Guide for Your Wrong Topics</p>
                  {remediation.studyGuide.focusPoints.length > 0 && (
                    <div className="mb-3">
                      <p className="text-xs text-slate-600 mb-1">Focus on these questions/concepts</p>
                      <div className="space-y-1">
                        {remediation.studyGuide.focusPoints.map((point, index) => (
                          <p key={`${point}-${index}`} className="text-xs text-slate-700">
                            {index + 1}. {point}
                          </p>
                        ))}
                      </div>
                  ...
                  {remediation.studyGuide.mistakeFixes.length > 0 && (
                    <div className="mb-3">
                      <p className="text-xs text-slate-600 mb-1">Wrong answer fixes</p>
                      <div className="space-y-2">
                        {remediation.studyGuide.mistakeFixes.slice(0, 3).map((fix, index) => (
                          <div key={`${fix.question}-${index}`} className="bg-white rounded-lg p-2.5 border border-slate-200">
                            <p className="text-xs text-slate-800">{fix.question}</p>
                            <p className="text-xs text-red-700 mt-1">Your answer: {fix.yourAnswer}</p>
                            <p className="text-xs text-emerald-700">Correct: {fix.correctAnswer}</p>
                            <p className="text-xs text-slate-600 mt-1">{fix.explanation}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {remediation.studyGuide.checklist.length > 0 && (
                    <div>
                      <p className="text-xs text-slate-600 mb-1">Quick revision checklist</p>
                      <div className="space-y-1">
                        {remediation.studyGuide.checklist.slice(0, 4).map((item, index) => (
                          <p key={`${item}-${index}`} className="text-xs text-slate-700">• {item}</p>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {remediation?.studyPlan?.length ? (
                <div className="bg-cyan-50 border border-cyan-200 rounded-xl p-4 mb-4">
                  <p className="text-sm font-medium text-cyan-700 mb-2">Auto-Created Study Plan</p>
                  <div className="space-y-2.5">
                    {remediation.studyPlan.map((day) => (
                      <div key={day.day} className="bg-white rounded-lg p-3 border border-cyan-100">
                        ...
                        <div className="space-y-1">
                          {day.tasks.map((task, index) => (
                            <p key={`${task}-${index}`} className="text-xs text-slate-600">• {task}</p>
                          ))}
                        </div>
                      </div>
                    ))}
      ...
      {wrong.length > 0 && (
        <div className="bg-white border border-blue-100 rounded-2xl p-5 shadow-sm mb-6">
          <h2 className="font-semibold text-slate-900 mb-4">Review Mistakes</h2>
          <div className="space-y-4">
            {wrong.map((a, i) => (
              <div key={i} className="bg-red-50 rounded-xl p-4 border border-red-100">
                <p className="text-sm text-slate-900 mb-3 leading-relaxed">{a.question}</p>
